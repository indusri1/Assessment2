...
// CODE HERE
function canWeDeliverTwo(zipCodesArray, zipCode){ 
...
deals.forEach(discountChange => {
    if (discountChange.title === '15% Off!') {
        discountChange.title = discountChange.title.replace('15% Off!', '10% Off!')
    }
})
...
deals.forEach(discountChange => {
    if (discountChange.title === 'Free Kids Meal with 2 Regular Entrees') {
        discountChange.desc = discountChange.desc.replace('March', 'April').trim()
    }
})
